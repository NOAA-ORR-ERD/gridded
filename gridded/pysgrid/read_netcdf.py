...
def find_grid_topology_var(nc):
    ...
    if topology_dimension != 2:
        msg = "Only 2 dimensions topology are supported.Got {0}".format
...
def parse_padding(padding_str, mesh_topology_var):
    ...
    for padding_match in padding_matches:
        ...
        grid_padding = GridPadding(
            mesh_topology_var=mesh_topology_var, face_dim=dim, node_dim=sub_dim, padding=padding_type
        )
    ...
    if len(padding_type_list) > 0:
        ...
        msg = "The netCDF file appears to have conform to SGRID conventions, but padding values cannot be found."
    ...
    def find_node_coordinates(self, node_dimensions):
        ...
        for nc_var in nc_vars.keys():
            ...
            if nc_var_dim_set == node_dim_set:
                ...
                elif any(y in name_lower for y in Y_COORDINATES) or any(
                    y in standard_name_lower for y in Y_COORDINATES
                ):
        ...
        for var_with_location in vars_with_location:
            ...
            try:
                ...
                for nc_var in nc_vars.keys():
                    ...
                    if (
                        nc_var_dim_set.issubset(location_var_dims)
                        and nc_var != var_with_location
                        and len(nc_var_dim_set) > 0
                    ):
                ...
                for potential_coordinate in potential_coordinates:
                    ...
                    try:
                        ...
                        pc_std_name = ""
                    if any(x in pc_name.lower() for x in X_COORDINATES) or any(
                        x in pc_std_name.lower() for x in X_COORDINATES
                    ):
                        x_coordinate = pc_name
                    elif any(y in pc_name.lower() for y in Y_COORDINATES) or any(
                        y in pc_std_name.lower() for y in Y_COORDINATES
                    ):  # noqa
                ...
                for lvc in lvc_split:
                    ...
                    try:
                        ...
                        var_coord_desc = ""
                    if (
                        "lon" in var_coord.name.lower()
                        or "longitude" in var_coord_standard_name.lower()
                        or "longitude" in var_coord_desc.lower()
                    ):
                        x_coordinate = lvc
                    elif (
                        "lat" in var_coord.name.lower()
                        or "latitude" in var_coord_standard_name.lower()
                        or "latitude" in var_coord_desc.lower()
                    ):
                        y_coordinate = lvc
                if len(lvc_split) == 3:
