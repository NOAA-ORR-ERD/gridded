...
from gridded.pysgrid.read_netcdf import NetCDFDataset, find_grid_topology_var, parse_padding
from gridded.pysgrid.utils import GridPadding, calculate_angle_from_true_east, pair_arrays
...
class SGrid:
    padding_slices = {"both": (1, -1), "none": (None, None), "low": (1, None), "high": (None, 1)}
    ...
    def load_grid(cls, nc):
        ...
        node_mask, center_mask, edge1_mask, edge2_mask = sa.get_masks(node_lon, center_lon, edge1_lon, edge2_lon)
        sgrid = cls(
            angles=angles,
            node_lon=node_lon,
            node_lat=node_lat,
    ...
    def info(self):
        ...
        """
        names = ", ".join([name for name, at in vars(self).items() if not name.startswith("_") if at is not None])

        msg = f"SGrid object with defined:\n    {names}"
    ...
    def _save_common_components(self, nc_file):
        ...
        center_lon = nc_file.createVariable(center_lon_obj.variable, center_lon_obj.dtype, center_lon_obj.dimensions)
        center_lat = nc_file.createVariable(center_lat_obj.variable, center_lat_obj.dtype, center_lat_obj.dimensions)
        ...
        try:
            ...
            grid_node_lon = nc_file.createVariable(node_lon_obj.variable, node_lon_obj.dtype, node_lon_obj.dimensions)
            node_lat_obj = getattr(self, node_lat)
            grid_node_lat = nc_file.createVariable(node_lat_obj.variable, node_lat_obj.dtype, node_lat_obj.dimensions)
        ...
        if hasattr(self, "angle"):
            ...
            grid_angle = nc_file.createVariable(angle_obj.variable, angle_obj.dtype, angle_obj.dimensions)
        ...
        for dataset_variable in self.variables:
            ...
            try:
                dataset_grid_var = nc_file.createVariable(
                    dataset_var_obj.variable, dataset_var_obj.dtype, dataset_var_obj.dimensions
                )
            except RuntimeError:
                continue
            else:
                ...
                if dataset_var_obj.x_axis is not None:
                    x_axis = f"X: {dataset_var_obj.x_axis}"
                    axes.append(x_axis)
                if dataset_var_obj.y_axis is not None:
                    y_axis = f"Y: {dataset_var_obj.y_axis}"
                    axes.append(y_axis)
                if dataset_var_obj.z_axis is not None:
                    z_axis = f"Z: {dataset_var_obj.z_axis}"
                    axes.append(z_axis)
                if axes:
    ...
    def _get_geo_mask(self, name):
        if name == "node":
            ...
            raise ValueError(f"Invalid grid name {name}")
    ...
    def get_efficient_slice(self, points=None, indices=None, location=None, _memo=False, _copy=False, _hash=None):
        """
    ...
    def locate_faces(self, points, _memo=False, _copy=False, _hash=None, use_mask=True):
        """
    ...
    def index_of(self, points, _memo=False, _copy=False, _hash=None, use_mask=True):
        return self.locate_faces(points=points, _memo=_memo, _copy=_copy, _hash=_hash, use_mask=use_mask)

    def locate_nearest(self, points, grid, _memo=False, _copy=False, _hash=None):
        points = np.asarray(points, dtype=np.float64)
        points = points.reshape(-1, 2)
    ...
    def apply_padding_to_idxs(self, idxs, padding=("none", "none")):
        """
        ...
        for dim, typ in enumerate(padding):
            if typ == "none" or typ == "high" or typ is None:
                ...
                raise ValueError(f"unrecognized padding type in dimension {dim}: {typ}")
        return idxs

    def get_padding_by_location(self, location):
        d = [
            (node_alternate_names, "node_padding"),
    ...
    def get_padding_slices(self, padding=("none", "none")):
        """
    ...
    def build_kdtree(self, grid="node"):
        ...
        try:
            ...
            raise ImportError(
                "The scipy package is required to use SGrid.locate_nearest\n -- nearest neighbor interpolation"
            )
        lon, lat = self._get_grid_vars(grid)
        if lon is None or lat is None:
            raise ValueError(
                f"{grid}_lon and {grid}_lat must be defined in order to create and use KDTree for this grid"
            )
    ...
    def build_celltree(self, use_mask=True):
        ...
        lon, lat = self.node_lon, self.node_lat
        if lon is None or lat is None:
            raise ValueError("node_lon and node_lat must be defined in order to create and use CellTree for this grid")

        if use_mask and (
            (self.node_mask is not None and self.node_mask is not False)
            or (self.center_mask is not None and self.center_mask is not False)
        ):
            if np.any(self.center_mask):
                cell_mask = gen_celltree_mask_from_center_mask(
                    self.center_mask, self.get_padding_slices(self.center_padding)
                )
            else:
                pass
            ...
            if lin_faces.shape[0] != cell_mask.size:
                raise ValueError(
                    "Could not match mask and faces array length. If padding is in use, please set self.center_padding"
                )
            ...
            lin_faces = np.array(
                [
                    np.array([[x, x + 1, x + x_size + 1, x + x_size] for x in range(0, x_size - 1, 1)]) + y * x_size
        ...
        if isinstance(lon, np.ma.MaskedArray) and lon.mask is not False and use_mask:
            lin_nodes = np.ascontiguousarray(
                np.column_stack((np.ma.compressed(lon[:]), np.ma.compressed(lat[:]))).reshape(-1, 2).astype(np.float64)
            )
        else:
    ...
    def nearest_var_to_points(
        ...
        if len(variable.shape) > 2:
            raise ValueError(
                "Variable has too many dimensions to \
    ...
    def interpolate_var_to_points(
        ...
        if location is None:
            ...
            warnings.warn(f"No location provided. Assuming data is on {location}")
        ...
        if location in center_alternate_names:
            ...
            m1 = np.logical_xor(
                self.get_variable_at_index(cm, center_idxs), self.get_variable_at_index(cm, center_idxs - u2_offset)
            )
            ...
            m1 = np.logical_xor(
                self.get_variable_at_index(cm, center_idxs), self.get_variable_at_index(cm, center_idxs - v2_offset)
            )
    ...
    def geo_to_logical(self, points, indices=None, _memo=False, _copy=False, _hash=None):
        """
        ...
        if indices is None:
            indices = self.locate_faces(points, _memo=_memo, _copy=_copy, _hash=_hash)
    ...
    def x_to_l(x, y, a, b):
        ...
        def quad_eqn(l, m, t, aa, bb, cc):
            ...
            det = np.ma.sqrt(k)
            m1 = (-bb - det) / (2 * aa)
            l1 = (x[t] - a[0][t] - a[2][t] * m1) / (a[1][t] + a[3][t] * m1)

            m2 = (-bb + det) / (2 * aa)
            l2 = (x[t] - a[0][t] - a[2][t] * m2) / (a[1][t] + a[3][t] * m2)
...
class SGridAttributes:
    ...
    def get_masks(self, node, center, edge1, edge2):
        ...
        mask_candidates = [
            ...
            if "mask" in var.name or (hasattr(var, "long_name") and "mask" in var.long_name)
        ]
