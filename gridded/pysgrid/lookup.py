"""
Created on Mar 24, 2015

@author: ayan
"""

LAT_GRID_CELL_CENTER_LONG_NAME = [
    "latitude of RHO-points",  # ROMS
    "X-coordinate of grid points",  # deltares
]

LON_GRID_CELL_CENTER_LONG_NAME = [
    "longitude of RHO-points",  # ROMS
    "Y-coordinate of grid points",  # deltares
]

LAT_GRID_CELL_NODE_LONG_NAME = [
    "latitude of PSI-points",  # ROMS
    "grid cell corners, latitude-coordinate",  # deltares
]

LON_GRID_CELL_NODE_LONG_NAME = [
    "longitude of PSI-points",  # ROMS
    "grid cell corners, longitude-coordinate",  # deltares
]

X_COORDINATES = ["x", "lon", "longitude"]

Y_COORDINATES = ["y", "lat", "latitude"]
