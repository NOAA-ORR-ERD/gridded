...
def load_from_varnames(filename, names_mapping, attribute_check=None, post_process=None):
    """
    ...
    for name, value in attribute_check:
        if nc.getncattr(name).lower() != value:
            raise ValueError(
                f'This does not appear to be a valid file:\nIt does not have the "{name}"="{value}"global attribute set'
            )
