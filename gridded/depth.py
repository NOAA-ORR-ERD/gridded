...
class DepthBase:
    ...
    def interpolation_alphas(
        ...
        **kwargs,
        ...
        indices = np.ma.MaskedArray(
            data=np.ones((sz,), dtype=np.int64) * self.surface_index, mask=np.zeros((sz,), dtype=bool)
        )
    ...
    @classmethod
    def _gen_varname(cls, filename=None, dataset=None, grid_dataset=None, names_list=None, std_names_list=None):
        """
...
class L_Depth(DepthBase):
    ...
    @classmethod
    def from_netCDF(
        cls, filename=None, dataset=None, grid_file=None, data_file=None, name=None, topology=None, terms=None, **kwargs
    ):
    ...
    def interpolation_alphas(
        ...
        **kwargs,
        ...
        bottom_boundary_condition = (
            self.default_bottom_boundary_condition if bottom_boundary_condition is None else bottom_boundary_condition
        )

        if data_shape is not None and data_shape[0] == 1 or self.num_levels == 1:  # surface only
            return super().interpolation_alphas(
                points, time, data_shape, _hash=_hash, extrapolate=extrapolate, **kwargs
            )
    ...
    def _find_required_depth_attrs(cls, filename, dataset=None, topology=None):
        ...
        init_args, gt = super()._find_required_depth_attrs(filename, dataset=dataset, topology=topology)
    ...
    def interpolation_alphas(
        ...
        **kwargs,
        ...
        if data_shape[0] != self.num_levels and data_shape[0] != self.num_layers:
            raise ValueError(
                "Cannot get depth interpolation alphas for data shape specified; does not fit r or w depth axis"
            )
...
class Depth:
    ...
    def __init__(self):
        raise NotImplementedError(
            "Depth is not meant to be instantiated. Please use the 'from_netCDF' or 'surface_only' function"
        )
    ...
    @classmethod
    def from_netCDF(cls, filename=None, dataset=None, data_file=None, grid_file=None, **kwargs):
        """
