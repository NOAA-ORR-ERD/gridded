    ...
    def interpolation_alphas(
        ...
        **kwargs,
        ...
        alphas = np.ma.MaskedArray(
            ...
            # mask=np.zeros(len(points), dtype=bool) # RDM: Redundant? Already default behavior
        )
        ...
        # CHECK: Where the bottom two lines removed???
        indices[above_surface] = 0 if ascending else (len(self.depth_levels) - 2)
        indices[below_bottom] = (len(self.depth_levels) - 2) if ascending else 0
...
class S_Depth(DepthBase):
        ...
        **kwargs,
        ...
        if cls == S_Depth:
            raise NotImplementedError(
                "S_Depth is not meant to be instantiated. Please use a subclass like ROMS_Depth or FVCOM_Depth"
            )
    ...
    def interpolation_alphas(
        ...
        **kwargs,
        ...
        if ((data_shape is not None and data_shape[0] == 1) 
        ...
        # the surface level will be 'within bounds' and the seafloor level 
        # will NOT be. 
        # the first input (depths) is treated as a single value (), 
        # the second input (transects) is treated as a 1D array of size (n), 
        # where n is the number of levels.  
        # The output of each operation is a scalar -> ()
        vf = np.vectorize(
            np.digitize, signature="(),(n)->()", excluded=["right"]
        )

        # Apply np.digitize on depths[i] and trasects[i,:] for every point 
        # from 0: N-1 and subract 1 to convert indexes from base of 1 to 
        # python indexing base of zero
        ...
        # Initialize alphas, but only at locations where transect mask is False
        # and using np.full to prevent uninitialized memory issues
        alphas = np.ma.MaskedArray(
            data = np.full(len(points), np.nan, dtype=np.float64), 
            mask = transects.mask[:, 0], # use mask
        )
        ...
        # Standardizing boundary slices using clip to keep take_along_axis safe
        # compute the remaining alphas, which should be for points within the 
        # depth interval transects is (n_points, n_levels): one water column 
        # per point, so the bracketing level depths must be taken from each 
        # point's own row. np.take without an axis would index the flattened 
        # array, reading every point's levels out of row 0.
        # masked or out-of-interval indices were already handled by the 
        # boundary conditions above and are never recomputed below, so 
        # fill/clip them to keep the take in bounds.
        ...
        # Explicitly mask out elements that are already handled by boundary 
        # masking.  Again, use mask to only count un-masked locations
        within_layer = np.isnan(alphas.data) & ~alphas.mask 
        

        # ~~~~~~~~~~. REVIEW:  IS THIS WHAT WE WANT? ~~~~~~~~~~~~~~~~
        # for a u-grid of 40-levels, index is 38, L0 is depth of level 38, 
        # L1 is depth of level 39, depths == depth for desired interpolated 
        # within_layer is True/False, depths[within_layer] - L0[within_layer] 
        # is the interpolation depth - depth of lower level. L1 - L0 is the 
        # depth distance between the two velocity points.  With ROMS, L1 > L0 
        # and L1 - L0 < 0, but the interpolation depth - lower boundary (L0) 
        # is also < 0
        # print(idx, L0, L1, depths[within_layer], points[-1][-1], 
        #       within_layer, depths[within_layer] - L0[within_layer])
        ...
        # Masked NaN values (only and if they exist) temporarily set to zero such 
        # that only un-masked NaN's get counted 
        if np.isnan(alphas.filled(0)).any():
            raise ValueError(
        ...
        if (alphas.filled(0) < 0).any() or (alphas.filled(0) > 1).any():
            print(f"depths.py[921], depth - lower level depth <0 or >1: {alphas}")
            # ~~~~~~~~~~. REVIEW:  IS THIS "IF" WHAT WE WANT? ~~~~~~~~~~~~~
            # Handle the case were
            # check to see if value is between upper-most grid depth and
            # air-sea interface.  If so, set alpha of lower boundary to 0
            # such that the alpha is 1 and the resulting value is 
            # 100% surface level 
...
class Depth():
    ...
    def surface_only(surface_index=-1, **kwargs):
        """
        If instantiated directly, this will always return a DepthBase 
        It is assumed index -1 is the surface index
        """
