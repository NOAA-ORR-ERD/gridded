...
class Grid_U(GridBase, UGrid):
    ...
    def _find_required_grid_attrs(cls, filename, dataset=None, grid_topology=None):
        ...
        init_args, gt = super()._find_required_grid_attrs(
            filename=filename, dataset=dataset, grid_topology=grid_topology
        )
...
class Grid_S(GridBase, SGrid):
    ...
    def _find_required_grid_attrs(cls, filename, dataset=None, grid_topology=None):
        ...
        gf_vars = dataset.variables if dataset is not None else get_dataset(filename).variables
        gf_vars = dict([(k.lower(), v) for k, v in gf_vars.items()])
        init_args, gt = super()._find_required_grid_attrs(filename, dataset=dataset, grid_topology=grid_topology)
...
class Grid_R(GridBase):
    ...
    def _find_required_grid_attrs(cls, filename, dataset=None, grid_topology=None):
        ...
        if len(node_lon.shape) != 1:
            raise ValueError(f"Too many dimensions in node_lon. Must be 1D, was {len(node_lon.shape)}D")
        if len(node_lat.shape) != 1:
            raise ValueError(f"Too many dimensions in node_lat. Must be 1D, was {len(node_lat.shape)}D")
        return init_args, gt
    ...
    def lonlat_to_yx(self, variable):
        ...
        if hasattr(variable, "dimensions"):
            if not all([k in self.dimensions for k in variable.dimensions[-2:]]):
                raise ValueError(
                    f"Dimension provided by variable is not compatible \
                                 with this Grid_R object. Provided: {variable.dimensions} \
        ...
        if not all([dlen in grid_shape for dlen in var_shape]):
            raise ValueError(f"Incompatible dimensions. Variable: {variable.shape}, Grid_R: {grid_shape}")
    ...
    def interpolate_var_to_points(
        self, points, variable, method="linear", indices=None, slices=None, mask=None, **kwargs
    ):
        try:
        ...
        interp_func = RegularGridInterpolator((y, x), variable, method=method, bounds_error=False, fill_value=0)
...
class Grid:
    ...
    def __init__(self):
        ...
        raise NotImplementedError(
            "Grid is not meant to be instantiated. "
    ...
    def from_netCDF(
        ...
        **kwargs,
        ...
        df, dg = parse_filename_dataset_args(
            filename=filename, dataset=dataset, data_file=data_file, grid_file=grid_file
        )

        cls = grid_type
        if grid_type is None or isinstance(grid_type, str) or not issubclass(grid_type, GridBase):
        ...
        if compliant is not None:
            ...
            init_args, gt = cls._find_required_grid_attrs(filename, dataset=dg, grid_topology=grid_topology)
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                raise ValueError(
                                    "Can not figure out what type of grid this is. "
