...
from gridded.pysgrid.utils import (
...
def test_pair_arrays():
    ...
    expected = np.array([[(1, 10), (2, 20)], [(3, 30), (4, 40)], [(5, 50), (6, 60)]])
...
def test_bearing_calculation():
    points = np.array([(-93.51105439, 11.88846735), (-93.46607342, 11.90917952)])
    point_1 = points[:-1, :]
    point_2 = points[1:, :]
...
def test_angle_from_true_east_calculation():
    vertical_1 = np.array([[-122.41, 37.78], [-122.33, 37.84], [-122.22, 37.95]])
    vertical_2 = np.array([[-90.07, 29.95], [-89.97, 29.84], [-89.91, 29.76]])
    vertical_3 = np.array([[-89.40, 43.07], [-89.49, 42.93], [-89.35, 42.84]])
    vertical_4 = np.array([[-122.41, 37.78], [-122.53, 37.84], [-122.67, 37.95]])
    centers = np.array((vertical_1, vertical_2, vertical_3, vertical_4))
    ...
    expected_values = np.array(
        [[0.7598, 0.9033, 0.9033], [-0.903, -0.994, -0.994], [-2.011, -0.719, -0.719], [-3.706, -3.926, -3.926]]
    )
    expected_shape = (4, 3)
    np.testing.assert_almost_equal(angle_from_true_east, expected_values, decimal=3)
