...
def test_locate_faces():
    ...
    assert (indices.data == ind_ans.data).all() and (indices.mask == ind_ans.mask).all()


def test_points_in_polys():
    points = np.array([[0, 0], [1, 0], [2, 0], [0, 1], [0, 2], [1, 2], [2, 2], [2, 1]])
    polygon = np.array(([0, 0], [2, 0], [2, 2], [0, 2])).reshape(1, 4, 2)
    pinp = np.array([points_in_polys(point.reshape(1, 2), polygon) for point in points]).reshape(-1)
...
def test_points_in_polys2():
    rectangle = np.array(([0, 0], [2, 0], [2, 2], [0, 2])).reshape(1, 4, 2)
    boundaries = np.array([[0, 0], [1, 0], [2, 0], [0, 1], [0, 2], [1, 2], [2, 2], [2, 1]])
    pinp = np.array([points_in_polys(point.reshape(1, 2), rectangle) for point in boundaries]).reshape(-1)
...
def test_nearest_neighbor():
    ...
    ind_ans = np.array([[0, 0], [0, 0], [0, 0], [1, 0], [1, 1], [2, 2], [2, 2]], dtype=np.int64)
...
def test_mirror_mask_values():
    values = np.ma.MaskedArray(
        data=[[9999, 7], [9999, -11], [150, 9999]], mask=[[True, False], [True, False], [False, True]]
    )
...
def test_compute_interpolant():
    ...
    values = np.ma.MaskedArray(
        data=[[-10, 10], [10, -10], [150, 200]], mask=[[False, False], [False, False], [True, True]]
    )
    ...
    values = np.ma.MaskedArray(
        data=[[-10, 10], [10, -10], [150, 200]], mask=[[False, False], [False, False], [True, False]]
    )
    alphas = np.ma.MaskedArray(
        data=[[0.75, 0.25], [0.5, 0.5], [0.5, 0.5]], mask=[[False, False], [False, False], [False, False]]
    )
