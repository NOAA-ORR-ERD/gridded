...
def gen_vortex_3D(filename=None):
    ...
    lin_faces = np.array(
        [
            np.array(
                [([lx, lx + x_size + 1, lx + 1], [lx, lx + x_size, lx + x_size + 1]) for lx in range(0, x_size - 1, 1)]
            )
            + ly * x_size
    ...
    if filename is not None:
        ...
        ds["time"][:] = nc4.date2num(tarr, f"hours since {t0}")
        ds["time"].setncattr("units", f"hours since {t0}")
...
def gen_sinusoid(filename=None):
    ...
    mask_rho[0, :] = True
    mask_rho[-1, :] = True
    rho = {"r_grid": (x, y, Z), "r_vel": (vx, vy, vz)}
    ...
    mask_psi[0, :] = True
    mask_psi[-1, :] = True
    psi = {"p_grid": (xc, yc, zc), "p_vel": (vxc, vyc, vzc)}
    ...
    mask_u[0, :] = True
    mask_u[-1, :] = True
    u = {"u_grid": (xu, yu, zu), "u_vel": (vzu, vxu, vzu)}
    ...
    mask_v[0, :] = True
    mask_v[-1, :] = True
    v = {"v_grid": (xv, yv, zv), "v_vel": (vyv, vzv, vzv)}
    ...
    if filename is not None:
        ds = nc4.Dataset(filename, "w", diskless=True, persist=True)
        for k, v in {"eta_psi": 24, "xi_psi": 4, "eta_rho": 25, "xi_rho": 5}.items():
            ds.createDimension(k, v)
        for k, v in {
            "lon_rho": ("xi_rho", "eta_rho", x),
        ...
        for k, v in {
            "mask_rho": ("xi_rho", "eta_rho", mask_rho),
            "mask_psi": ("xi_psi", "eta_psi", mask_psi),
            "mask_u": ("xi_rho", "eta_psi", mask_u),
            "mask_v": ("xi_psi", "eta_rho", mask_v),
        }.items():
