...
def quad_and_triangle():
    ...
    fname = tempfile.mktemp(suffix=".nc", dir=test_files, prefix="tmp_quad_and_triangle")
    ...
    mesh2_face_nodes = ds.createVariable(
        "Mesh2_face_nodes", "int32", dimensions=("nMesh2_face", "nMaxMesh2_face_nodes"), fill_value=999999
    )
    mesh2_face_nodes_attrs = dict(
        cf_role="face_node_connectivity", long_name="Maps every face to its corner nodes.", start_index=0
    )
    ...
    mesh2_edge_nodes_attrs = dict(
        cf_role="edge_node_connectivity", long_name="Maps every edge to the two nodes that it connects.", start_index=0
    )
    ...
    mesh2_face_edges = ds.createVariable(
        "Mesh2_face_edges", "int32", dimensions=("nMesh2_face", "nMaxMesh2_face_nodes"), fill_value=999999
    )
    mesh2_face_edges_attrs = dict(
        cf_role="face_edge_connectivity", long_name="Maps every face to its edges.", start_index=0
    )
    update_with_attributes(mesh2_face_edges, mesh2_face_edges_attrs)

    mesh2_face_links = ds.createVariable(
        "Mesh2_face_links", "int32", dimensions=("nMesh2_face", "nMaxMesh2_face_nodes"), fill_value=999999
    )
    ...
    mesh2_edge_face_links = ds.createVariable(
        "Mesh2_edge_face_links", "int32", dimensions=("nMesh2_edge", "two"), fill_value=-999
    )
    ...
    mesh2_node_x = ds.createVariable("Mesh2_node_x", "double", dimensions=("nMesh2_node",))
    mesh2_node_x_attrs = dict(standard_name="longitude", long_name="Longitude of 2D mesh nodes.", units="degrees_east")
    update_with_attributes(mesh2_node_x, mesh2_node_x_attrs)

    mesh2_node_y = ds.createVariable("Mesh2_node_y", "double", dimensions=("nMesh2_node",))
    mesh2_node_y_attrs = dict(standard_name="latitude", long_name="Latitude of 2D mesh nodes.", units="degrees_north")
    update_with_attributes(mesh2_node_y, mesh2_node_y_attrs)

    # Optional mesh face and edge coordinate variables
    mesh2_face_x = ds.createVariable("Mesh2_face_x", "double", dimensions=("nMesh2_face",))
    ...
    mesh2_face_y = ds.createVariable("Mesh2_face_y", "double", dimensions=("nMesh2_face",))
    ...
    mesh2_face_xbnds_attrs = dict(
        standard_name="longitude",
        long_name=("Longitude bounds of 2D mesh face (i.e. corner coordinates)."),
        units="degrees_east",
    )
    ...
    mesh2_edge_x = ds.createVariable("Mesh2_edge_x", "double", dimensions=("nMesh2_edge",))
    mesh2_edge_x_attrs = dict(
        standard_name="longitude",
        long_name=("Characteristic longitude of 2D mesh edge (e.g. midpoint of the edge)."),
        units="degrees_east",
    )
    update_with_attributes(mesh2_edge_x, mesh2_edge_x_attrs)

    mesh2_edge_y = ds.createVariable("Mesh2_edge_y", "double", dimensions=("nMesh2_edge",))
    mesh2_edge_y_attrs = dict(
        standard_name="latitude",
        long_name=("Characteristic latitude of 2D mesh edge (e.g. midpoint of the edge)."),
        units="degrees_north",
    )
    ...
    mesh2_face_nodes[:] = [[0, 1, 2, 999999], [1, 3, 4, 2]]
    mesh2_edge_nodes[:] = [[0, 1], [1, 2], [2, 0], [1, 3], [3, 4], [4, 2]]
    mesh2_face_edges[:] = [[0, 1, 2, 999999], [3, 4, 5, 1]]
    mesh2_face_links[:] = [[1, -1, -1, -1], [0, -1, -1, -1]]
    mesh2_edge_face_links[:] = [[0, -999], [0, 1], [0, -999], [1, -999], [1, -999], [1, -999]]
    ...
    mesh2_face_xbnds[:, :] = np.array([[0.0, 1.0, 0.5, 9.9692099683868690e36], [1.0, 1.5, 1.0, 0.5]], dtype="double")
    mesh2_face_ybnds[:] = [[0.0, 0.0, 1.0, 9.9692099683868690e36], [0.0, 1.0, 2.0, 1.0]]
