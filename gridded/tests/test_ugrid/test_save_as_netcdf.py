...
def ncds():
    ...
    if not os.path.isdir(temp_files):
        os.mkdir(temp_files)
    fname = os.path.join(temp_files, f"temp_{file_counter[0]}.nc")
    ...
    if os.path.isfile(fname):
        os.remove(fname)
    ncds = netCDF4.Dataset(fname, mode="w", clobber=True, format="NETCDF4")
    yield (fname, ncds)
...
def nc_has_variable(ds, var_name):
    ...
    if var_name in ds.variables:
        ...
        print(f"{var_name} is not a variable in the Dataset")
        return False
...
def test_simple_write(two_triangles, ncds):
    ...
    assert nc_var_has_attr_vals(
        ds,
        "mesh",
        {"cf_role": "mesh_topology", "topology_dimension": 2, "long_name": "Topology data of 2D unstructured mesh"},
    )
    ds.close()
...
def test_write_with_depths(two_triangles, ncds):
    ...
    assert nc_var_has_attr_vals(
        ds, "depth", {"coordinates": "mesh1_node_lon mesh1_node_lat", "location": "node", "mesh": "mesh1"}
    )
    ds.close()
...
def test_write_with_edge_data(two_triangles, ncds):
    ...
    assert nc_var_has_attr_vals(
        ds,
        "flux",
        {"coordinates": "mesh2_edge_lon mesh2_edge_lat", "location": "edge", "units": "m^3/s", "mesh": "mesh2"},
    )
    assert np.array_equal(ds.variables["mesh2_edge_lon"], gds.grid.edge_coordinates[:, 0])
    assert np.array_equal(ds.variables["mesh2_edge_lat"], gds.grid.edge_coordinates[:, 1])
    ds.close()
...
def test_write_with_bound_data(two_triangles, ncds):
    ...
    assert nc_var_has_attr_vals(ds, "mesh3", {"boundary_node_connectivity": "mesh3_boundary_nodes"})
    assert nc_var_has_attr_vals(
        ds,
        "bnd_cond",
        {
...
def test_write_everything(twenty_one_triangles, ncds):
    ...
    assert nc_has_variable(ds, "mesh")
    assert nc_has_variable(ds, "depth")
    assert nc_var_has_attr_vals(ds, "depth", {"coordinates": "mesh_node_lon mesh_node_lat", "location": "node"})
    assert nc_has_variable(ds, "u")
    assert nc_has_variable(ds, "v")
    assert nc_var_has_attr_vals(
        ds, "u", {"coordinates": "mesh_face_lon mesh_face_lat", "location": "face", "mesh": "mesh"}
    )
    assert nc_var_has_attr_vals(
        ds, "v", {"coordinates": "mesh_face_lon mesh_face_lat", "location": "face", "mesh": "mesh"}
    )
    assert nc_has_variable(ds, "flux")
    assert nc_var_has_attr_vals(
        ds, "flux", {"coordinates": "mesh_edge_lon mesh_edge_lat", "location": "edge", "units": "m^3/s", "mesh": "mesh"}
    )
    assert nc_has_variable(ds, "mesh")
    assert nc_has_variable(ds, "bnd_cond")
    assert nc_var_has_attr_vals(ds, "mesh", {"boundary_node_connectivity": "mesh_boundary_nodes"})
    assert nc_var_has_attr_vals(
        ds,
        "bnd_cond",
        {
