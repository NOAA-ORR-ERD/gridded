...
@pytest.fixture()
def sg_data():
    # filename = os.path.join(TEST_DATA, "arakawa_c_test_grid.nc")
    filename = TEST_DATA / "arakawa_c_test_grid.nc"
...
@pytest.fixture()
def ug_data():
    filename = TEST_DATA / "tri_ring.nc"
    return filename, nc.Dataset(filename)
...
class TestGrid_S:
    ...
    def test_masked_grid(self, sg_data, sg_topology):
        ...
        pts = {"on": [1.1, 33.6], "off": [0.9, 30.6], "masked": [1.1, 31.6]}
...
def example_rg():
    ...
    lats = np.array((0, 2, 3, 4, 5, 7, 9))
    gt = {"node_lon": "lon", "node_lat": "lat"}
    rg = Grid_R(node_lon=lons, node_lat=lats, grid_topology=gt)
    return rg
